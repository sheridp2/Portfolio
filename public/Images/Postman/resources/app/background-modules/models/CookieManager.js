...
const { ensureProperUrl, getURLProps } = require('../../universal-utils/url');
...
class CookieManager {
  ...
  pushCookies (cookies, url, finalCallback) {
    var sentUrl = getURLProps(ensureProperUrl(url)),
        ...
        matchingCookies = _.filter(cookies, ['domain', domain]),
        responseCookies = [],  // these sync back to the view
        minuteTimezoneOffset = (new Date()).getTimezoneOffset(),
        millisOffset = minuteTimezoneOffset * 60000,
        existingCookiesForDomain;

    _.forEach(matchingCookies, (cookie) => {
      if (!cookie) {
        return;
      }
      ...
      (callback) => {
        ...
        async.each(responseCookies, (cookie, cb) => {
          if (cookie.expirationDate) {
            cookie.expirationDate = parseInt(((new Date(cookie.expirationDate)).getTime() + millisOffset) / 1000);
          }
